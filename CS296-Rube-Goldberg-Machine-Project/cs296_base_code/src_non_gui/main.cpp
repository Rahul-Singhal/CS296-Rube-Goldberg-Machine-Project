/*modifications by Rahul Singhal for lab4*/
...
int main(int argc, char** argv)
{
  ...
  const b2World * myworld;
  myworld = test->get_world();
  const b2Profile& p = myworld->GetProfile();
	
	int totalIterations = 0;
	int i = 0;
	while(argv[1][i] != '\0'){
		totalIterations = totalIterations*10 + (int)(argv[1][i++]-'0');
	}
	float collisionTime = 0;
	float velocityUpdates = 0;
	float positionUpdates = 0;
	...
	for(int32 i = 0 ; i < totalIterations ; i++) {
		totalTime += p.step; test->step(&settings);
		collisionTime += p.collide;
		velocityUpdates += p.solveVelocity;
		positionUpdates += p.solvePosition;
		}
	printf("Average time per step is %7.4f ms\n", totalTime/totalIterations);
	printf("Average time for collisions is %7.4f ms\n", collisionTime/totalIterations);
	printf("Average time for velocity updates is %7.4f ms\n", velocityUpdates/totalIterations);
	printf("Average time for position updates is %7.4f ms\n", positionUpdates/totalIterations);
	//printf("This is from the source code branch made by Rahul Singhal from Group 18 for CS296 Lab 04.\n");
	
	struct timeval tv2;
	gettimeofday(&tv2,NULL);
	//double t2 = (float)tv1.tv_sec*1000 + (float)(tv1.tv_usec)/1000;
	//printf("%f\n", t2);
	double diff = (float)((tv2.tv_sec-tv1.tv_sec)*1000 + (float)(tv2.tv_usec-tv1.tv_usec)/1000);
	printf("Total time for loop is %7.4f ms\n", diff);
